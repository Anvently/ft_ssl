...
static size_t put_ptr_str(char *buffer, size_t size, void *ptr) {
    ...
    if (nwrite != 2)
        return (nwrite);
    nwrite += ft_putunbr_base_buffer((size_t)ptr, buffer + 2, size - 2,
                                     "0123456789abcdef");
    return (nwrite);
}

static size_t parse_format(char *buffer, size_t size, const char *format,
                           va_list args) {
    size_t len = 0;
    size_t nwrite;
    ...
    while (*format && len + 1 < size) {
        if (*format == '%') {
            ...
            case 'd':
                nwrite = ft_putnbr_buffer(va_arg(args, int), buffer + len,
                                          size - len);
                len += nwrite;
                format += 2;
                break;
            ...
            case 'u':
                nwrite = ft_putunbr_buffer(va_arg(args, unsigned int),
                                           buffer + len, size - len);
                len += nwrite;
                format += 2;
                break;
            ...
            case 's':
                nwrite = put_str_buffer(buffer + len, size - len,
                                        va_arg(args, char *));
                len += nwrite;
                format += 2;
                break;
            ...
            case 'p':
                nwrite =
                    put_ptr_str(buffer + len, size - len, va_arg(args, void *));
                len += nwrite;
                format += 2;
                break;

            case 'x':
                nwrite = ft_putunbr_base_buffer(va_arg(args, unsigned int),
                                                buffer + len, size - len,
                                                "0123456789abcdef");
                len += nwrite;
                format += 2;
            ...
            case 'l':
                switch (*(format + 2)) {
                case 'd':
                    nwrite = ft_putnbr_buffer(va_arg(args, long int),
                                              buffer + len, size - len);
                    len += nwrite;
                    format += 3;
                    break;
                ...
                case 'u':
                    nwrite = ft_putunbr_buffer(va_arg(args, long unsigned int),
                                               buffer + len, size - len);
                    len += nwrite;
                    format += 3;
                    break;
                ...
                case 'x':
                    nwrite = ft_putunbr_base_buffer(
                        va_arg(args, long unsigned int), buffer + len,
                        size - len, "0123456789abcdef");
                    len += nwrite;
                    format += 3;
                    break;
